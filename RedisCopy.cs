...
namespace redis_copy
{
    internal class RedisCopy
    {
        ...
        public RedisCopy(string? sourceConStr, string? destConStr, string? destFlushData, string? dbIndex)
        {
            ...
            if (string.IsNullOrEmpty(destFlushData)) throw new ArgumentNullException(nameof(destFlushData));
            if (string.IsNullOrEmpty(dbIndex)) throw new ArgumentNullException(nameof(dbIndex));

            var sourceOptions = ConfigurationOptions.Parse(sourceConStr);
            sourceOptions.AllowAdmin = true;//Run info to get keyspace need enable the Admin
            sourcecon = ConnectionMultiplexer.Connect(sourceOptions);

            var destOptions = ConfigurationOptions.Parse(destConStr);
            destOptions.AllowAdmin = true;
            destcon = ConnectionMultiplexer.Connect(destOptions);

            sourceEndpoint = sourceOptions.EndPoints[0].ToString().Replace("Unspecified/", ""); ;
            destEndpoint = destOptions.EndPoints[0].ToString().Replace("Unspecified/", ""); ;

            flushdest = Convert.ToBoolean(destFlushData);
            dbToCopy = Convert.ToInt32(dbIndex);
        }
        public void Copy()
        {
